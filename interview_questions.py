...
# # # # # Lists are represented with square brackets ['dolly', 5, 0.10], while tuples are represented with parantheses ('rahul', 8, 0.97).
...
# # # The Python dictionary is the only type of mapping in the base typeset. Mappings do not maintain any left-to-right position order; 
# # # they support access to stored data by key, as well as type-specific method calls
...
# # # User-defined functions are functions that are created by the user, 
# # # and they can be created to do anything that the user wants them to do.
...
# # # A Pass statement in Python is used when we cannot decide what to do in our code, 
# # # but we must type something to make it syntactically correct.
...
# # # How would you convert JSON to Python objects?
# # # The json library in Python can help you convert JSON data into Python objects. 
# # # The process is known as decoding. You can use the json.loads() function to decode JSON data. 
...
# # What is _init_?
# # _init_ is a contructor method in Python and is automatically called to allocate memory when a new object/instance is created. 
# # All classes have a _init_ method associated with them. 
